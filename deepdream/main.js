...
async function ensureModel(){
 ...
 disposeModel();const cfg=profileConfig();status(`TensorFlow準備中… (${cfg.name})`);
 try{
  await withTimeout(tf.ready(),10000,'TensorFlow初期化が10秒を超えました');
  if(tf.getBackend()!=='webgl'){
   status('WebGLを初期化中…');
   try{await withTimeout(tf.setBackend('webgl'),8000,'WebGL初期化失敗');await tf.ready()}catch(e){console.warn(e);status('WebGL不可。CPUで続行します（かなり遅い）');await tf.setBackend('cpu');await tf.ready()}
  }
  status(`モデル取得中… (${cfg.name})`);
  model=await withTimeout(mobilenet.load({version:cfg.version,alpha:cfg.alpha}),45000,'モデル取得が45秒を超えました。通信を確認して再読み込みしてください');
  status('中間層を準備中…');
  const m=model.model;
...
async function runDream({record=false}={}){
 if(!current)return status('先に画像を選んでください');if(busy)return status('処理中です。停止するか完了まで待ってください');busy=true;$('run').disabled=true;abort=false;
