...
import {buildWorld,canOccupy,movePlayer,roomAt,EYE} from './world.js';
...
export async function startArchitecture({Renderer=THREE.WebGLRenderer,entryURL=location.href,reviewLabel=''}={}){
  ...
  const inspectionLight=new THREE.PointLight(0xbac9ad,3.4,6.2,2);inspectionLight.position.set(3.4,2.35,-1.0);scene.add(inspectionLight);
  const rearLight=new THREE.PointLight(0xc77c54,2.0,4.0,2);rearLight.position.set(-3.8,1.9,-5.0);scene.add(rearLight);
  ...
  function resize(){
    ...
    camera.aspect=innerWidth/innerHeight;camera.fov=innerHeight>innerWidth?83:68;camera.updateProjectionMatrix();
  }
  ...
  function move(f,s,amount){
    if(!running||paused||contextLost)return;
    const dx=(Math.sin(player.yaw)*f+Math.cos(player.yaw)*s)*amount;
    const dz=(-Math.cos(player.yaw)*f+Math.sin(player.yaw)*s)*amount;
    movePlayer(player,dx,dz);
  }
  ...
  function findTarget(point){
    if(point){
      ray.setFromCamera(new THREE.Vector2(point.x/innerWidth*2-1,1-point.y/innerHeight*2),camera);
      const hit=visibleHit(ray.ray.origin,ray.ray.direction);
      return hit?world.targets.find(t=>t.mesh===hit.object):null;
    }
    camera.getWorldDirection(direction);let best=null,bestScore=.93;
    for(const t of world.targets){
      const v=t.position.clone().sub(camera.position),distance=v.length();if(distance<.08||distance>2.45)continue;
      v.normalize();const score=direction.dot(v);if(score<=bestScore)continue;
      const hit=visibleHit(camera.position,v);if(hit&&hit.object!==t.mesh&&hit.distance<distance-.07)continue;
  ...
  function inspect(point){
    ...
    if(target.text)notice(target.text);
    if(target.href){save();controls.reset();document.exitPointerLock?.();setTimeout(()=>location.assign(new URL(target.href,entryURL).href),120);}
  }
  ...
  function refreshUI(fps,p95){
    const room=roomAt(player.x,player.z);$('location').textContent=room?.name||'廊下';
  ...
  function animate(t){
    ...
    if(running&&!paused){const {f,s}=controls.read(dt);if(f||s)move(f,s,dt*1.52);}
    updateCamera();
    const calm=$('calm').checked;
    const pulse=calm?1:(elapsed%13.7>12.9&&elapsed%13.7<13.08?.35:1);
    for(const m of world.flickerMaterials)m.emissiveIntensity=1.8*pulse;
    if(running&&!paused){scan+=dt;if(scan>.1){scan=0;active=findTarget();$('interact').hidden=!active;if(active)$('target-label').textContent=active.label;}}
    else {$('interact').hidden=true;active=null;}
    renderer.render(scene,camera);
    if(running&&!paused){frames.push(raw*1000);uiClock+=raw;saved+=dt;
      if(uiClock>=1.5){const sorted=[...frames].sort((a,b)=>a-b),p95=sorted[Math.floor(sorted.length*.95)]||16.7,fps=frames.length*1000/Math.max(1,frames.reduce((a,b)=>a+b,0));refreshUI(fps,p95);
        if(quality==='auto'){slow=fps<32?slow+1:0;fast=fps>54?fast+1:0;if(slow>=2&&dpr>.7){dpr=Math.max(.7,dpr-.15);resize();slow=0;}if(fast>=12&&dpr<Math.min(devicePixelRatio||1,touch?1.15:1.45)){dpr=Math.min(dpr+.1,devicePixelRatio||1,touch?1.15:1.45);resize();fast=0;}}
        frames=[];uiClock=0;
      }
  ...
  $('enter').disabled=false;$('enter').textContent='入る';
  $('enter').onclick=()=>{running=true;controls.enabled=true;$('gate').hidden=true;$('.top');document.querySelector('.top').hidden=false;showControls();canvas.focus({preventScroll:true});refreshUI(0,0);notice('祭祀技師事務所。');};
}
