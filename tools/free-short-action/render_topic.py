...
UA = "FreeShortStudio-GitHubAction/1.1 (open-source trivia video renderer)"
...
def http_get(
    ...
    *,
    ...
    for attempt in range(attempts):
        ...
        try:
            wait = float(retry_after) if retry_after else min(1.5 * (2**attempt), 24.0)
        except ValueError:
            wait = min(1.5 * (2**attempt), 24.0)
        wait += random.uniform(0.2, 0.8)
        print(
            f"WARN HTTP {r.status_code} for {_without_query(url)}; retry {attempt + 1}/{attempts} in {wait:.1f}s",
            flush=True,
        )
        time.sleep(wait)
...
def resolve_wikipedia_page(topic: str, lang: str = "ja") -> dict:
    ...
    data = http_get(
        ...
        params={
            "action": "query",
            "prop": "extracts|langlinks|images",
            "titles": title,
            "redirects": 1,
            "explaintext": 1,
            "imlimit": 50,
    ...
    return {
        ...
        "source_page": f"https://{lang}.wikipedia.org/wiki/{quote(resolved.replace(' ', '_'))}",
    }
...
def commons_asset_from_page(page: dict) -> Asset | None:
    ...
    if not any(
        x in liclow
        for x in ["public domain", "cc0", "cc by", "cc-by", "cc-by-sa", "attribution", "gfdl"]
    ):
        return None
    media = ii.get("thumburl") or ii.get("url") or ""
    original = ii.get("url") or ""
    if not media:
        return None
    return Asset(
        file_title=title,
        media_url=_without_query(media),
        original_url=_without_query(original),
...
def commons_info_for_titles(file_titles: list[str], thumb_width: int = 1200) -> list[Asset]:
    out: list[Asset] = []
    for batch in chunks(file_titles, 20):
        data = http_get(
            COMMONS_API,
            params={
                "action": "query",
...
def commons_search(query: str, limit: int = 30, thumb_width: int = 1200) -> list[Asset]:
    data = http_get(
        COMMONS_API,
        params={
            "action": "query",
...
def collect_assets(page: dict, needed: int) -> list[Asset]:
    target = max(needed * 2, needed + 6)
    assets = commons_info_for_titles(page["images"])
    for q in [
        page["en_title"],
        page["title"],
        f'{page["en_title"]} history',
        f'{page["en_title"]} photograph',
    ]:
        assets = dedupe_assets(assets)
        if len(assets) >= target:
            break
        try:
            assets.extend(commons_search(q, limit=max(target, 16)))
        except Exception as exc:
            print(f"WARN Commons search {q!r}: {exc}", flush=True)
        time.sleep(0.5)
    assets = dedupe_assets(assets)
    minimum = max(4, min(needed, 6))
    if len(assets) < minimum:
        raise RuntimeError(
            f"Only {len(assets)} reusable real images found; refusing generated fallback"
        )
    return assets


def _download_one(asset: Asset, dest: Path) -> bool:
    urls = []
    for u in [asset.media_url, asset.original_url]:
        u = _without_query(u)
        if u and u not in urls:
            urls.append(u)
    for url in urls:
        try:
            r = http_get(url, timeout=80, attempts=5)
            ...
            if len(r.content) < 5_000:
                raise RuntimeError(f"image response too small: {len(r.content)} bytes")
            dest.write_bytes(r.content)
            asset.local_path = str(dest)
            print(
                f"downloaded {asset.title} -> {dest} ({len(r.content)} bytes)",
                flush=True,
            )
            return True
        except Exception as exc:
            print(
                f"WARN download failed for {asset.title} from {url}: {type(exc).__name__}: {exc}",
                flush=True,
            )
            time.sleep(1.0 + random.uniform(0.2, 0.7))
    return False


def download_assets(candidates: list[Asset], outdir: Path, needed: int) -> list[Asset]:
    outdir.mkdir(parents=True, exist_ok=True)
    downloaded: list[Asset] = []
    for asset in candidates:
        if len(downloaded) >= needed:
            break
        low = (asset.media_url + " " + asset.original_url + " " + asset.file_title).lower()
        suffix = ".png" if ".png" in low else ".webp" if ".webp" in low else ".jpg"
        dest = outdir / f"asset_{len(downloaded) + 1:02d}{suffix}"
        if _download_one(asset, dest):
            downloaded.append(asset)
            time.sleep(1.25 + random.uniform(0.25, 0.75))
        else:
            print(f"WARN skipping unusable asset: {asset.title}", flush=True)
            time.sleep(1.0)
    if len(downloaded) < needed:
        raise RuntimeError(
            f"Downloaded only {len(downloaded)}/{needed} reusable real images; refusing generated fallback"
        )
    return downloaded
...
async def synthesize_one(text: str, out: Path, voice: str, rate: str) -> None:
    ...
    for attempt in range(4):
        try:
            ...
            last = exc
            wait = 1.5 * (2**attempt) + random.uniform(0.1, 0.8)
            print(f"WARN Edge-TTS retry {attempt + 1}/4 in {wait:.1f}s: {exc}", flush=True)
...
def write_ass(topic: str, texts: list[str], durations: list[float], out: Path) -> None:
    ...
    for i, (text, dur) in enumerate(zip(texts, durations), 1):
        start, end = t, t + dur
        lines.append(
            f"Dialogue: 0,{ass_time(start)},{ass_time(end)},Topic,,0,0,0,,{escape_ass(topic)}\n"
        )
        lines.append(
            f"Dialogue: 0,{ass_time(start)},{ass_time(end)},Counter,,0,0,0,,{i:02d}/{total:02d}\n"
        )
        lines.append(
            f"Dialogue: 0,{ass_time(start)},{ass_time(end)},Caption,,0,0,0,,{escape_ass(text)}\n"
        )
        t = end
...
def make_bgm(
    ...
    for i in range(total):
        t = i / sample_rate
        drone = 0.13 * math.sin(2 * math.pi * 110 * t) + 0.06 * math.sin(
            2 * math.pi * 165 * t
        )
        phase = t % beat
        pulse = 0.0
...
def render_scene(image: Path, voice: Path, out: Path, duration: float) -> None:
    # Keep the whole real image visible. A blurred full-canvas copy only fills the unused portrait space.
    fc = (
        "[0:v]split=2[bg][fg];"
        "[bg]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,"
        "gblur=sigma=34,eq=brightness=-0.20:saturation=0.70[bg2];"
        "[fg]scale=1000:1210:force_original_aspect_ratio=decrease[fg2];"
        "[bg2][fg2]overlay=(W-w)/2:165,"
        "drawbox=x=0:y=1450:w=1080:h=470:color=black@0.50:t=fill,format=yuv420p[v]"
    )
    run(
        [
            "ffmpeg",
...
def final_mix(body: Path, ass: Path, bgm: Path, out: Path) -> None:
    escaped = (
        str(ass.resolve()).replace("\\", "/").replace(":", "\\:").replace("'", "\\'")
    )
    fc = (
        f"[0:v]ass='{escaped}'[v];"
...
def make_contact_sheet(video: Path, out: Path, duration: float) -> None:
    ...
    run(
        [
            ...
            str(video),
            "-vf",
            f"fps=1/{interval},scale=270:480,tile=3x2",
            "-frames:v",
            "1",
            str(out),
        ]
...
def main() -> None:
    ...
    candidates = collect_assets(page, len(texts))
    ...
    write_ass(page["title"], texts, durations, ass)
    total = sum(durations)
    bgm = work / "bgm.wav"
    make_bgm(bgm, total)
    ...
    for i, a in enumerate(assets, 1):
        credits.append(
            f"{i}. {a.title} | {a.creator or 'unknown'} | {a.license} | {a.source_page}"
        )
