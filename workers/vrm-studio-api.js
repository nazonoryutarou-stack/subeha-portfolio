...
const handleTranscribe = async (request, env) => {
  ...
  if (audio.size > MAX_AUDIO_BYTES) {
    return json({error: 'audio file exceeds the 25MB transcription upload limit'}, 413);
  }
  ...
  const durationMs = Math.max(
    0,
    Math.round(Number(payload?.duration || 0) * 1000),
...
const handleGenerateImage = async (request, env) => {
  ...
  return json({
    model: 'gpt-image-2',
    data: [{b64_json: item.b64_json, revised_prompt: item.revised_prompt || null}],
  });
};
